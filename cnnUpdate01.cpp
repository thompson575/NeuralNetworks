...
// outputs
//   List containing
//     bias   the biases on completion
//     weight the weights on completion
...
List cfit_valid_nn( NumericMatrix X, 
  ...
  for( int iter = 0; iter < nIter; iter++ ) {
    ...
    // update fitted solution
    if( (iter == 0) | (validHistory[iter] < floss) ) {
    ...
    for(int i = nX; i < nNodes; i++) bias[i]   -= eta * db[i] / nr;
    // report progress if appropriate
    if((iter + 1) % trace == 0) Rprintf("%i %f %f\n", iter+1, 
       lossHistory[iter], validHistory[iter]);
  }
  // return the results
